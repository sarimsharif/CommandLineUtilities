function Install-WizTree {
    Write-Host "Installing WizTree via winget..."
    try {
        winget install --id AntibodySoftware.WizTree -e
        Write-Host "WizTree installed successfully!"
    } catch {
        Write-Host "Failed to install WizTree via winget. Downloading from the official website..."
        $url = "https://download.diskanalyzer.com/files/wiztree_4_16_setup.exe"
        $output = "$env:TEMP\wiztree_setup.exe"
        Invoke-WebRequest -Uri $url -OutFile $output
        Start-Process $output
        Write-Host "WizTree installer downloaded and launched. Please complete the installation manually."
    }
